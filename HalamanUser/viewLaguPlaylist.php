...
$id_playlist = $_GET['id'];
$query = mysqli_query($koneksi,"SELECT * FROM playlist_name WHERE id_playlist_name = '$id' ");
...
<head>
  ...
  <title>Playlist - <?= $data2['playlist_name']; ?></title>
          ...
          <div class="col-12"> 
           ...
           <div class="table-responsive" style="overflow: scroll; scrollbar-width: thin;">
            <table class="table table-hover" id="tabel-data">
              ...
              <tbody>
                <?php 
                $query = mysqli_query($koneksi,"SELECT * FROM playlist INNER JOIN music ON playlist.id_music = music.id_music WHERE id_playlist_name = '$id_playlist' ");
                $i = 0;
                while ($data = mysqli_fetch_array($query)) : 
                  $i++;
                  ?>
